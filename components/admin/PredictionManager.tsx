// components/admin/PredictionManager.tsx
'use client';
...
export default function PredictionManager() {
  ...
  const isAdmin = publicKey?.toString() === process.env.NEXT_PUBLIC_ADMIN_WALLET;
  ...
  if (!isAdmin) return <div className="text-red-500">Admin only</div>;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-[#1a1d29] p-6 rounded-2xl border border-gray-800">
        ...
        <div className="flex gap-3">
          <button onClick={triggerAutoSettle} className="flex items-center gap-2 px-4 py-2 bg-orange-500/20 text-orange-400 rounded-lg hover:bg-orange-500/30 border border-orange-500/30">
            <Clock className="w-4 h-4" />
            Auto-Settle BTC
          </button>
          <button onClick={() => setShowCreate(!showCreate)} className="flex items-center gap-2 px-6 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-semibold">
      ...
      {showCreate && (
        <form onSubmit={createPrediction} className="bg-[#1a1d29] p-6 rounded-2xl border border-gray-800 space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 disabled:opacity-50 text-white font-bold py-3 rounded-lg">
            ...
            <div className="flex justify-between items-start">
              ...
              {pred.status === 'active' && pred.category === 'manual' && (
                <div className="flex gap-2 ml-4">
                  <button onClick={() => settlePrediction(pred.id, 'up')} className="px-4 py-2 bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 border border-green-500/30 font-semibold">
                    <Check className="w-4 h-4 inline mr-1" />
                    UP Wins
                  </button>
                  <button onClick={() => settlePrediction(pred.id, 'down')} className="px-4 py-2 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 border border-red-500/30 font-semibold">
